...
class epv_test_validate_composer extends BaseTest
{
	...
	private function validateLicense(ComposerFileInterface $file)
	{
		...
		$this->addMessageIfBooleanTrue(isset($json['license']) && ($json['license'] !== 'GPL-2.0-only' || $json['license'] !== 'GPL-2.0'), Output::ERROR, 'It is required to use the "GPL-2.0-only" as license. Other licenses are not allowed as per the extension database policies.');
	}
