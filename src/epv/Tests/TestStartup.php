...
class TestStartup
{
    ...
    /**
     * Run the testsuite with the current directory.
     */
    ...
    private function printResults()
    {
        ...
        if ($this->output->getMessageCount(Output::FATAL))
        {
            $this->output->writeln('<fatal>' . str_repeat(' ', strlen($found_msg)) . '</fatal>');
            $this->output->writeln('<fatal>Validation: FAILED' . str_repeat(' ', strlen($found_msg) - 18) . '</fatal>');
